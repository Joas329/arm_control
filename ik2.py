...
class InverseKinematics:
    def __init__(self, urdf_file_path):
        urdf_xml_string = self.load_file(urdf_file_path)
    ...
    def load_file(self, file_path):
        if file_path.endswith('.xacro'):
            return self.process_xacro(file_path)
        elif file_path.endswith('.urdf'):
            return self.read_urdf(file_path)
        else:
            raise ValueError("Unsupported file format. Please use .urdf or .xacro files.")

    def read_urdf(self, file_path):
        with open(file_path, 'r') as file:
            return file.read()

    def process_xacro(self, file_path):
        # You need to have xacro installed and be running this in a ROS environment
        import subprocess
        command = ['xacro', file_path]
        try:
            output = subprocess.run(command, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            return output.stdout.decode()
        except subprocess.CalledProcessError as e:
            print("Failed to process xacro file:")
...
if __name__ == "__main__":
    ...
    urdf_file_path = './description/yurs_base_station.urdf.xacro'
